...
router.post("/vendor/login", passport.authenticate("vendor",
  {
    ...
    successFlash: "Welcome to SmallBizz!"
  }), function (req, res) {
  });
  ...
  async.waterfall([
    ...
    function (token, user, done) {
      ...
      var mailOptions = {
        to: user.email,
        from: "YOUR_EMAIL_ID",
        subject: "SmallBizz Password Reset",
  ...
  async.waterfall([
    ...
    function (user, done) {
      ...
      var mailOptions = {
        to: user.email,
        from: "YOUR_EMAIL_ID",
        subject: "SmallBizz: Your password has been changed",
        text: "Hello,\n\n" +
    ...
    if (err) {
      ...
      if (res.locals.currentUser.isCustomer) {
        ...
        if (res.locals.currentUser.username != foundVendor.username) {
          ...
          res.render("vendor/v_show", { vendor: foundVendor, error: "Your profile is not verified yet and will not be visible to others. SmallBizz team will contact you soon for verification." });
        }
      }
    } else {
